...
impl Workspace {
    ...
    pub fn render(&mut self) -> io::Result<()> {
        ...
        let tip = self
            .tip
            .as_deref()
            .unwrap_or(" Ctrl+B сайдбар  Shift+Tab фокус ");
        let status = format!(
            ...
            dirty,
            quit_hint,
            truncate_str(tip, cols.saturating_sub(60))
        );
        ...
        match key {
            ...
            Key::Enter => {
                if let Some(e) = self.tree.get(self.tree_sel) {
                    ...
                    if self.doc.dirty {
                        self.tip = Some("Сначала Ctrl+S (сохранить) или Ctrl+Q дважды.".into());
                        return;
                    }

                    if let Err(err) = self.doc.load_file(e.path.clone()) {
                        self.tip = Some(format!("Ошибка: {err}"));
                    } else {
                        self.focus = Focus::Editor;
                    }
                }
